...
public class RouteNavigator extends AbstractFeature {
        ...
        case ROTATION: 
            {
                ...
                RotationConfiguration config = new RotationConfiguration(new Target(this.rotationTarget), this.getLookTime(nextPoint), this.rotationType, null).followTarget(true);
    ...
    protected void onPacketReceive(Packet<?> packet) {
        ...
        Vec3 pos = new Vec3(positionPacket.change().position().x + (positionPacket.relatives().contains(Relative.X) ? currentPlayerPos.x : 0), positionPacket.change().position().y + (positionPacket.relatives().contains(Relative.Y) ? currentPlayerPos.y : 0), positionPacket.change().position().z + (positionPacket.relatives().contains(Relative.Z) ? currentPlayerPos.z : 0));
