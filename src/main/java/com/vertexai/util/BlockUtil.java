...
import com.vertexai.util.helper.heap.MinHeap;
...
public class BlockUtil {
    ...
    public static List<BlockPos> getWalkableBlocksAround(BlockPos playerPos) {
        return getWalkableBlocksAround(playerPos, 1);
    }

    public static List<BlockPos> getWalkableBlocksAround(BlockPos playerPos, int radius) {
    ...
    public static int getMiningTime(int stateId, final int miningSpeed) {
        try {
            ...
            return (int) Math.ceil((DEFAULT_BLOCK_STRENGTH * 30) / (float) miningSpeed) + Vertex.config().delays.tickGlideOffset;
        }
    }

    public static int getMiningTime(BlockState state, final int miningSpeed) {
        return (int) Math.ceil((getBlockStrength(state) * 30) / (float) miningSpeed) + Vertex.config().delays.tickGlideOffset;
    }
    ...
    public static boolean canWalkBetween(CalculationContext ctx, BlockPos start, BlockPos end) {
        ...
        return !com.vertexai.pathfinder.util.BlockUtil.INSTANCE.bresenham(ctx, start, end);
    }
