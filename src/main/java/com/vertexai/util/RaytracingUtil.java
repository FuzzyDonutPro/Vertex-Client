...
public class RaytracingUtil {
    ...
    public static HitResult raytraceWithEntities(Vec3 from, Vec3 to, Predicate<Entity> predicate) {
        ...
        AABB searchAABB = new AABB(from, to).inflate(1.0);

        for (Entity entity : mc.level.getEntities(mc.player, searchAABB, e -> e != mc.player && e.isAlive() && predicate.test(e))) {
    ...
    public static HitResult findCrosshairTarget(Entity camera, double blockInteractionRange, double entityInteractionRange, float tickProgress) {
        ...
        EntityHitResult entityHitResult = ProjectileUtil.getEntityHitResult(camera, startVec, endVec, box, net.minecraft.world.entity.Entity::isPickable, e);
