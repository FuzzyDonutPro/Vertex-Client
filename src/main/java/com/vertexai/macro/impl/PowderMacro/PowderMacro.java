package com.vertexai.macro.impl.PowderMacro;

import lombok.Getter;
import lombok.Setter;
import com.vertexai.Vertex;
import com.vertexai.macro.FeatureManager;
import com.vertexai.macro.features.misc.AutoChestUnlocker;
...
import com.vertexai.macro.impl.PowderMacro.states.PowderMacroState;
import com.vertexai.macro.impl.PowderMacro.states.StartingState;
...
public class PowderMacro extends AbstractMacro {
    ...
    private String getEnvironmentInvalidReason() {
        ...
        if (!isInsideAreaLock()) {
            return "Outside selected area lock (" + getAreaLockName(Vertex.config().powderMacro.areaLock) + ").";
        }
        return null;
    }

    public boolean isInsideAreaLock() {
        int areaLock = Vertex.config().powderMacro.areaLock;
        if (areaLock == 0) {
            return true;
        }
        if (mc.player == null) {
    ...
    public boolean ensureMinerRunning() {
        ...
        this.blockMiner.setWaitThreshold(Vertex.config().general.oreRespawnWaitThreshold * 1000);
    ...
    public Optional<BlockPos> findNearbyChest() {
        if (Vertex.config().powderMacro.ignoreChests || mc.player == null || mc.level == null) {
    ...
    public void beginChestSolve(BlockPos chestPos) {
        ...
        String miningTool = Vertex.config().general.miningTool == null ? "" : Vertex.config().general.miningTool;
    ...
    private MineableBlock[] getTargetBlocks() {
        if (Vertex.config().powderMacro.powderType == 1) {
