...
import com.vertexai.macro.impl.CommissionMacro.states.CommissionMacroState;
...
public class CommissionHUD extends TextHud {
    ...
    protected boolean shouldShow() {
        ...
        if (!commissionMacro.isEnabled()) return false;
        boolean macroTypeCondition = (Vertex.config().general.macroType == 0);
        if (!macroTypeCondition) {
            return false;
        }

        if (Vertex.config().commission.dwarvenCommission.showDwarvenCommHUDOutside) {
            return true;
        }
