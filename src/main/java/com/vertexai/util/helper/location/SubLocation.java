package com.vertexai.util.helper.location;
...
public enum SubLocation {
    ...
    //<editor-fold desc="Galatea/Backwater Bayou">
    ...
    TANGLEBURG_BANK("Tangleburg Bank"),
    ...
    private static final Map<String, SubLocation> aliasToLocationMap = new HashMap<>();

    static {
        for (SubLocation location : SubLocation.values()) {
            nameToLocationMap.put(location.getName(), location);
        }
        aliasToLocationMap.put("Khazad-dm", KHAZADDUM);
        aliasToLocationMap.put("Dj Vu Alley", DEJA_VU_ALLEY);
        aliasToLocationMap.put("Stillgore Chteau", STILLGORE_CHATEAU);
        aliasToLocationMap.put("Glacite Mineshaft", GLACITE_MINESHAFT);
        aliasToLocationMap.put("Moonglades Edge", MOONGLADES_EDGE);
        aliasToLocationMap.put("Tangleburgs Path", TANGLEBURGS_PATH);
    }
    ...
    public static SubLocation fromName(String name) {
        ...
        SubLocation location = nameToLocationMap.get(normalized);
        if (location != null) {
            return location;
        }
        location = aliasToLocationMap.get(normalized);
        if (location != null) {
            return location;
        }
        return KNOWHERE;
