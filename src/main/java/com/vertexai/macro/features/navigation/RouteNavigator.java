package com.vertexai.macro.features.navigation;
...
import com.vertexai.util.helper.RotationConfiguration.RotationType;
...
public class RouteNavigator extends AbstractFeature {
    ...
    private int getLookTime(final RouteWaypoint waypoint) {
        if (this.rotationType == RotationType.SERVER) {
            return Vertex.config().delays.delayAutoAotvServerRotation;
        }
        if (waypoint.getTransportMethod().ordinal() == 0) {
            return Vertex.config().delays.delayAutoAotvLookDelay;
        }
        return Vertex.config().delays.delayAutoAotvEtherwarpLookDelay;
    }
    ...
    protected void onPacketReceive(Packet<?> packet) {
        ...
        Vec3 pos = new Vec3(
                positionPacket.change().position().x + (0),
                positionPacket.change().position().y + (0),
                positionPacket.change().position().z + (0)
        );
