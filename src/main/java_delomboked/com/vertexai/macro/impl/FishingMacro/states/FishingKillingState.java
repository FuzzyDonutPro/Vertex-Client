...
import com.vertexai.util.FishingUtil;
...
public class FishingKillingState implements FishingMacroState {
    ...
    public FishingMacroState onTick(FishingMacro macro) {
        ...
        int minStriderCount = Math.min(
                Vertex.config().fishing.galateaFishing.galateaStriderMinCount,
                Vertex.config().fishing.galateaFishing.galateaStriderMaxCount
        );
        ...
        Set<Integer> nearbyStriderIds = collectNearbyStriderIds(Vertex.config().fishing.galateaFishing.galateaStriderScanRadius);
    ...
    private void runFishingLoop(long now) {
        ...
        long maxWaitMs = Math.max(5L, Vertex.config().fishing.generalFishing.maxWaitSeconds) * 1000L;
    ...
    public String getFishingLoopStateName() {
        return fishingLoopState.getName();
    }
    ...
    private long performCapKillRightClick(long now) {
        if (shouldUseSlayerFastSwapEnabled() && now >= nextCapKillSlayerAssistAtMs) {
            String slayerWeapon = Vertex.config().fishing.galateaFishing.galateaFishingWeapon;
    ...
    private long getCastActionDelayMs() {
        int base = Math.max(100, Vertex.config().fishing.generalFishing.castDelayMs);
        int randomizer = Math.max(0, Vertex.config().fishing.generalFishing.castDelayRandomizerMs);
        return base + (randomizer > 0 ? random.nextInt(randomizer + 1) : 0);
    }

    private long getCombatActionDelayMs() {
        int base = Math.max(50, Vertex.config().fishing.galateaFishing.galateaCombatDelayMs);
        int randomizer = Math.max(0, Vertex.config().fishing.galateaFishing.galateaCombatDelayRandomizerMs);
        return base + (randomizer > 0 ? random.nextInt(randomizer + 1) : 0);
    }
    ...
    private boolean shouldUseSlayerFastSwapEnabled() {
        ...
        if (!Vertex.config().fishing.galateaFishing.galateaFastWeaponSwap) {
            return false;
        }
