...
public enum Commission {
    ...
    MITHRIL_MINER("Mithril Miner", SubLocation.RAMPARTS_QUARRY), TITANIUM_MINER("Titanium Miner", SubLocation.RAMPARTS_QUARRY), UPPER_MITHRIL("Upper Mines Mithril", SubLocation.UPPER_MINES), UPPER_TITANIUM("Upper Mines Titanium", SubLocation.UPPER_MINES), ROYAL_MITHRIL("Royal Mines Mithril", SubLocation.ROYAL_MINES), ROYAL_TITANIUM("Royal Mines Titanium", SubLocation.ROYAL_MINES), LAVA_MITHRIL("Lava Springs Mithril", SubLocation.LAVA_SPRINGS), LAVA_TITANIUM("Lava Springs Titanium", SubLocation.LAVA_SPRINGS), CLIFFSIDE_MITHRIL("Cliffside Veins Mithril", SubLocation.CLIFFSIDE_VEINS), CLIFFSIDE_TITANIUM("Cliffside Veins Titanium", SubLocation.CLIFFSIDE_VEINS), RAMPARTS_MITHRIL("Rampart\'s Quarry Mithril", SubLocation.RAMPARTS_QUARRY), RAMPARTS_TITANIUM("Rampart\'s Quarry Titanium", SubLocation.RAMPARTS_QUARRY), GOBLIN_SLAYER("Goblin Slayer", SubLocation.GOBLIN_BURROWS), GLACITE_WALKER_SLAYER("Glacite Walker Slayer", SubLocation.GREAT_ICE_WALL), MINES_SLAYER("Mines Slayer", SubLocation.GOBLIN_BURROWS), COMMISSION_CLAIM("Claim Commission", SubLocation.THE_FORGE),  // theres no set location for this so yea
    REFUEL("Refuel Drill", SubLocation.FORGE_BASIN);
    // theres no set location for this so yea
    // Do not want this
...
//  STAR_CENTRY_SLAYER("Star Sentry Puncher	"),
    ...
    private static final Set<SubLocation> GENERIC_MINER_LOCATIONS = EnumSet.of(SubLocation.RAMPARTS_QUARRY, SubLocation.UPPER_MINES, SubLocation.ROYAL_MINES, SubLocation.LAVA_SPRINGS, SubLocation.CLIFFSIDE_VEINS);
    ...
    static {
        ...
        Map<SubLocation, RouteWaypoint[]> veinsMap = new EnumMap<SubLocation, RouteWaypoint[]>(SubLocation.class) {
            {
                put(SubLocation.FORGE_BASIN, new RouteWaypoint[] {new RouteWaypoint(-9, 144, -20, WaypointType.WALK)});
                put(SubLocation.THE_FORGE, new RouteWaypoint[] {new RouteWaypoint(44, 134, 21, WaypointType.WALK), new RouteWaypoint(58, 197, -11, WaypointType.WALK), new RouteWaypoint(171, 149, 33, WaypointType.WALK), new RouteWaypoint(-75, 152, -11, WaypointType.WALK), new RouteWaypoint(-132, 173, -53, WaypointType.WALK)});
                put(SubLocation.CLIFFSIDE_VEINS, new RouteWaypoint[] {new RouteWaypoint(93, 144, 51, WaypointType.WALK), new RouteWaypoint(28, 130, 26, WaypointType.WALK)});
                put(SubLocation.ROYAL_MINES, new RouteWaypoint[] {new RouteWaypoint(115, 153, 83, WaypointType.WALK), new RouteWaypoint(141, 152, 27, WaypointType.WALK)});
                ...
                put(SubLocation.RAMPARTS_QUARRY, new RouteWaypoint[] {new RouteWaypoint(-41, 138, -13, WaypointType.WALK), new RouteWaypoint(-58, 146, -18, WaypointType.WALK)});
                put(SubLocation.UPPER_MINES, new RouteWaypoint[] {new RouteWaypoint(-111, 166, -74, WaypointType.WALK), new RouteWaypoint(-145, 206, -30, WaypointType.WALK)});
    ...
    public static List<Commission> getBestCommissionFrom(Map<Commission, Double> commissionsWithProgress) {
        ...
        for (Map.Entry<Commission, Double> entry : commissionsWithProgress.entrySet()) {
            ...
            if (best == null || effectiveScore < bestEffectiveScore || (effectiveScore == bestEffectiveScore && progress > bestProgress) || (effectiveScore == bestEffectiveScore && progress == bestProgress && baseScore < bestBaseScore)) {
                best = candidate;
        ...
        Logger.sendLog("Selected commission by weighted score: " + best.getName() + " (base=" + bestBaseScore + ", progress=" + Math.round(bestProgress * 100.0) + "%, effective=" + String.format(Locale.US, "%.2f", bestEffectiveScore) + ")");
    ...
    private static RouteWaypoint selectBestWaypoint(Collection<RouteWaypoint> candidates) {
        ...
        return candidates.stream().min(Comparator.comparingInt((RouteWaypoint waypoint) -> countNearbyRealPlayers(waypoint.toVec3(), avoidRadiusSq)).thenComparingDouble(waypoint -> waypoint.toVec3().distanceToSqr(playerPos))).orElseGet(() -> candidates.iterator().next());
    }
    ...
    private static int countNearbyRealPlayers(Vec3 target, double radiusSq) {
        ...
        return (int) mc.level.players().stream().filter(player -> !player.is(mc.player)).filter(Player::isAlive).filter(player -> !EntityUtil.isNpc(player)).filter(player -> player.position().distanceToSqr(target) <= radiusSq).count();
    }
    ...
    public RouteWaypoint getWaypoint() {
        ...
        if (this == MITHRIL_MINER || this == TITANIUM_MINER) {
            ...
            Collection<SubLocation> locationsToCheck = !targetLocations.isEmpty() ? targetLocations : GENERIC_MINER_LOCATIONS;
            List<RouteWaypoint> allWaypoints = locationsToCheck.stream().map(VEINS::get).filter(Objects::nonNull).flatMap(Arrays::stream).collect(java.util.stream.Collectors.toList());
