...
public class ConfigManager {
    ...
    private static final Path CONFIG_PATH = FabricLoader.getInstance()
            ...
            .resolve("Vertex")
            .resolve("Vertex.json");
    private static final Path BACKUP_PATH = CONFIG_PATH.resolveSibling("Vertex.json.bak");
    private static final Path TEMP_PATH = CONFIG_PATH.resolveSibling("Vertex.json.tmp");
     ...
     */
    private static boolean normalizeColourOptions(VertexConfig config) {
    ...
    private static VertexConfig readConfigWithDefaults(Path path) {
        ...
        try {
            ...
            JsonObject merged = gson.toJsonTree(new VertexConfig()).getAsJsonObject();
            deepMergeInto(merged, parsed.getAsJsonObject());

            VertexConfig config = gson.fromJson(merged, VertexConfig.class);
            if (config == null) {
                throw new IllegalStateException("Gson returned null config");
            }
            return config;
        } catch (Exception e) {
            Vertex.LOGGER.warn("Failed to read config from {}: {}", path, e.toString());
            Vertex.LOGGER.debug("Config read failure details for {}", path, e);
            return null;
        }
    }
    ...
    public void firstLoad() {
        VertexConfig loadedConfig = readConfigWithDefaults(CONFIG_PATH);
        if (loadedConfig == null) {
            loadedConfig = readConfigWithDefaults(BACKUP_PATH);
            if (loadedConfig == null) {
                Vertex.LOGGER.warn("No readable config found; creating defaults at {}", CONFIG_PATH);
                loadedConfig = new VertexConfig();
            } else {
                Vertex.LOGGER.warn("Recovered config from backup: {}", BACKUP_PATH);
            }

            VertexClient.config = loadedConfig;
            saveConfig();
        } else {
            VertexClient.config = loadedConfig;
        }
    ...
    public void saveConfig() {
        synchronized (ioLock) {
            try {
                ...
                byte[] jsonBytes = gson.toJson(VertexClient.config).getBytes(StandardCharsets.UTF_8);
                Files.write(
                        TEMP_PATH,
                        jsonBytes,
                ...
                Vertex.LOGGER.error("Failed to save config: {}", CONFIG_PATH, e);
                try {
                    Files.deleteIfExists(TEMP_PATH);
                } catch (IOException cleanupError) {
                    Vertex.LOGGER.warn("Failed to clean up temp config file: {}", TEMP_PATH, cleanupError);
                }
            }
        }
    }
