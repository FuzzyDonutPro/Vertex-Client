...
public class ChatAIHandler {
    ...
    private static void generateReply(String contextMessage) {
        ...
        if (apiKey == null || apiKey.isEmpty()) {
            Minecraft.getInstance().player.displayClientMessage(Component.literal("§c[Vertex AI] API Key is missing in config!"));
            return;
        }
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    if (Minecraft.getInstance().player != null) {
                        client.execute(() -> {
                            client.getNetworkHandler().sendChatMessage(reply.trim().replace("\n", " "));
                        });
                    }
                } else {
                    Minecraft.getInstance().player.displayClientMessage(Component.literal("§c[Vertex AI] API Error: " + response.statusCode()));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
