...
public class PlayerFailsafe extends AbstractFailsafe {
    ...
    public boolean onTick() {
        ...
        List<Entity> players = StreamSupport.stream(mc.level.entitiesForRendering().spliterator(), false).filter(entity -> entity instanceof AbstractClientPlayer && entity != mc.player && !EntityUtil.isNpc(entity)).collect(Collectors.toList());
    ...
    private boolean isPlayerLookingAtMe(Entity player) {
        ...
        Vec3 playerToMeVec = new Vec3(mc.player.getX() - player.getX(), mc.player.getBoundingAABB().minY + mc.player.getEyeHeight(mc.player.getPose()) - (player.getY() + player.getEyeHeight(player.getPose())), mc.player.getZ() - player.getZ());
        double d0 = playerToMeVec.length();
        playerToMeVec = playerToMeVec.normalize();
        double dot = lookVec.dot(playerToMeVec);
        double fovCosine = Math.cos(Math.toRadians(30.0F));
    ...
    @lombok.Generated
    public static PlayerFailsafe getInstance() {
        return PlayerFailsafe.instance;
    }
}
