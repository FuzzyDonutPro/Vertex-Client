...
public class AutoWarp extends AbstractFeature {
    ...
    protected void onTick() {
        ...
        if (!isOnHypixelNetwork()) {
            sendError("Not connected to Hypixel. AutoWarp stopped.");
            this.stop(Error.FAILED_TO_WARP);
            return;
        }
    ...
    protected void onChat(String message) {
        ...
        if (message.contains(this.noWarpScroll)) {
            ...
            sendError("Please consume the " + getCurrentTargetName() + " travel scroll.");
        }
    }
    ...
    private String getWarpCommand(Location location) {
        ...
        if (command.startsWith("/") || command.equals("l")) {
            return command;
        }
        return "warp " + command;
    }
    ...
    private String getCurrentTargetName() {
        if (this.targetSubLocation != null) {
            return this.targetSubLocation.getName();
        }
        if (this.targetLocation != null) {
            return this.targetLocation.getName();
        }
