package com.vertexai.util.helper.route;
...
public class Route {
    ...
    public Optional<RouteWaypoint> getClosest(final BlockPos pos) {
        return this.waypoints.stream()
                .min(Comparator.comparingDouble(wp -> wp.toVec3().distanceToSqr(Vec3.atCenterOf(pos))));
    }
    ...
    public void drawRoute() {
        for (int i = 0; i < this.waypoints.size(); i++) {
            ...
            String name = currWaypoint.getTransportMethod().getName();
            RenderUtil.drawBlock(
                    currWaypoint.toBlockPos(),
                    RenderUtil.parseColor(Vertex.config().routeMiner.routeBuilderNodeColor, new java.awt.Color(0, 255, 255, 100))
            );
            ...
            RenderUtil.drawLine(
                    ...
                    currWaypoint.toVec3().add(0.5, 0.5, 0.5),
                    RenderUtil.parseColor(Vertex.config().routeMiner.routeBuilderTracerColor, new java.awt.Color(255, 255, 255, 150))
            );
        }
    }
