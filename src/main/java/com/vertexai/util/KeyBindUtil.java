...
import com.vertexai.mixin.client.KeyMappingAccessor;
...
public class KeyBindUtil {
    ...
    private static long lastRightClickMs = 0L;  // Global BadPacketsC guard
    private static long lastLeftClickMs  = 0L;  // Global BadPacketsC guard
    ...
    public static void rightClick() {
        ...
        if (now - lastRightClickMs < 50) return;  // BadPacketsC: min 1-tick (50ms) gap
        lastRightClickMs = now;
        KeyMapping.click(((KeyMappingAccessor) mc.options.keyUse).getBoundKey());
    }

    public static void leftClick() {
        long now = System.currentTimeMillis();
        if (now - lastLeftClickMs < 50) return;  // BadPacketsC: min 1-tick (50ms) gap
        lastLeftClickMs = now;
        KeyMapping.click(((KeyMappingAccessor) mc.options.keyAttack).getBoundKey());
    }

    public static void middleClick() {
        KeyMapping.click(((KeyMappingAccessor) mc.options.keyPickItem).getBoundKey());
    }

    public static void onTick(KeyMapping key) {
        if (mc.screen == null) {
            KeyMapping.set(((KeyMappingAccessor) key).getBoundKey(), true);
        }
    }
    ...
    public static void holdThese(boolean withAttack, KeyMapping... keyBinding) {
        // Never move while a GUI/inventory screen is open
        if (mc.screen != null) {
            stopMovement();
            return;
        }
    ...
    public static boolean contains(KeyMapping[] keyBinding, KeyMapping key) {
        ...
        for (KeyMapping keyBind : keyBinding) {
            if (keyBind != null && ((KeyMappingAccessor) keyBind).getBoundKey().getValue() == boundKey.getValue())
                return true;
        }
        return false;
    }
