...
public class MiningMacro extends AbstractMacro {
    ...
    private boolean handleRefuelIfNeeded() {
        ...
        if (fuel <= LOW_FUEL_THRESHOLD && !AutoDrillRefuel.getInstance().isRunning()) {
            ...
            AutoDrillRefuel.FuelType[] fuelTypeMap = {AutoDrillRefuel.FuelType.VOLTA, AutoDrillRefuel.FuelType.OIL_BARREL, AutoDrillRefuel.FuelType.SUNFLOWER_OIL};
    ...
    public void onTick() {
        ...
        if (!isMining) {
            miner.setWaitThreshold(Vertex.config().general.oreRespawnWaitThreshold * 1000);
            miner.start(blocksToMine, miningSpeed, pickaxeAbility, determinePriority(), Vertex.config().general.miningTool);
            isMining = true;
            log("Started mining with speed: " + miningSpeed);
            log("Started mining with pickaxe ability: " + pickaxeAbility.getName());
        }
        handleMining();
    }
    ...
    private void handleGettingStats() {
        ...
        if (miningSpeedRetrievalTask.getError() != null) {
            super.disable("Failed to get stats with the following error: " + miningSpeedRetrievalTask.getError());
            return;
        }
        if (pickaxeAbilityRetrievalTask.getError() != null) {
            super.disable("Failed to get pickaxe ability with the following error: " + pickaxeAbilityRetrievalTask.getError());
            return;
        }
        miningSpeed = miningSpeedRetrievalTask.getResult();
        pickaxeAbility = Vertex.config().general.usePickaxeAbility ? pickaxeAbilityRetrievalTask.getResult() : BlockMiner.PickaxeAbility.NONE;
    }
    ...
    private void handleMining() {
        ...
        case NO_PICKAXE_ABILITY: 
            disable("Cannot find messages for pickaxe ability! Either enable any pickaxe ability in HOTM or enable chat messages. You can also disable pickaxe ability in configs.");
            break;
        }
    }

    private void setBlocksToMineBasedOnOreType() {
        log("Setting blocks to mine based on ore type: " + Vertex.config().miningMacro.oreType);
        switch (Vertex.config().miningMacro.oreType) {
        case 0: 
            blocksToMine = new MineableBlock[] {MineableBlock.GRAY_MITHRIL, MineableBlock.GREEN_MITHRIL, MineableBlock.BLUE_MITHRIL, MineableBlock.TITANIUM};
            break;
        case 1: 
    ...
    private int[] determinePriority() {
        if (Vertex.config().miningMacro.oreType == 0) {
            return new int[] {Vertex.config().miningMacro.mineGrayMithril ? Vertex.config().miningMacro.mithrilPriorityGrayDefault : 0, Vertex.config().miningMacro.mineGreenMithril ? Vertex.config().miningMacro.mithrilPriorityGreenDefault : 0, Vertex.config().miningMacro.mineBlueMithril ? Vertex.config().miningMacro.mithrilPriorityBlueDefault : 0, Vertex.config().miningMacro.mineTitanium ? Vertex.config().miningMacro.mithrilPriorityTitaniumDefault : 0};
        }
