...
public class CommissionDebugMode extends AbstractFeature {
    ...
    public void start() {
        ...
        send("Commission Debug Mode Â§aENABLEDÂ§r - Showing route and enhanced mining logic visualizations");
        // Play enable sound
        if (mc.player != null) {
    ...
    public void stop() {
        ...
        send("Commission Debug Mode Â§cDISABLED");
        // Play disable sound
        if (mc.player != null) {
    ...
    public void setCustomTarget(BlockPos target, String name) {
        ...
        this.enabled = true;
        calculateAllRoutes();
        send("Â§aDebug path set to: Â§f" + name + " " + target);
    }
    ...
    public void clearCustomTarget() {
        ...
        calculateAllRoutes();
        send("Â§aCleared custom debug target");
    }
    ...
    private void calculateAllRoutes() {
        ...
        if (customTarget != null) {
            try {
                List<RouteWaypoint> pathNodes = GraphHandler.instance.findPathFrom(
                        GRAPH_NAME, playerPos, new RouteWaypoint(customTarget, com.vertexai.util.helper.route.WaypointType.WALK)
                );
                ...
                if (!pathNodes.isEmpty()) {
                    ...
                    send("Â§cCould not find path to " + customTargetName);
                    customRoute = null;
                }
            } catch (Exception e) {
    ...
    private void simulateMiningLogic() {
        ...
        if (!activeCommission.name().contains("Miner") && !activeCommission.name().contains("Mithril") && !activeCommission.name().contains("Titanium")) {
        ...
        int[] priorityToUse;
        if (activeCommission.name().contains("Titanium")) {
            priorityToUse = titaniumPriority;
        } else {
            priorityToUse = Vertex.config().commission.dwarvenCommission.prioritiseTitanium ? prioritiseTitanium : mithrilPriority;
        }
    ...
    protected void onWorldRender(WorldRenderContextWrapper context) {
        ...
        if (simulatedTargetBlock != null) {
            ...
            // Draw text
            RenderUtil.drawText("Â§6TARGET\nÂ§f" + simulatedTargetReason,
        ...
        RenderUtil.drawText("Â§aYOU", playerPos.getX() + 0.5, playerPos.getY() + 2.5, playerPos.getZ() + 0.5, 1.0F);
        ...
        for (Map.Entry<Commission, Route> entry : calculatedRoutes.entrySet()) {
            ...
            Color color = routeColors.getOrDefault(
                    commission,
                    RenderUtil.parseColor(Vertex.config().debug.debugRouteColor, new Color(0, 255, 0, 180))
            );
    ...
    private void drawRouteWithInfo(Route route, Commission commission, Color color) {
        ...
        for (int i = 0; i < route.size(); i++) {
            ...
            if (i > 0) {
                ...
                RenderUtil.drawLine(
                        prevWaypoint.toVec3().add(0.5, 0.5, 0.5),
                        waypoint.toVec3().add(0.5, 0.5, 0.5),
                        color
                );
            }
        }
        ...
        String endLabel = "Â§l" + commission.name() + "Â§r Â§7(" + String.format("%.1f", distance) + "m)";
    ...
    private void drawRouteWithCustomInfo(Route route, String name, Color color) {
        ...
        for (int i = 0; i < route.size(); i++) {
            ...
            if (i > 0) {
                ...
                RenderUtil.drawLine(prev.toVec3().add(0.5, 0.5, 0.5), waypoint.toVec3().add(0.5, 0.5, 0.5), color);
            }
        }

        BlockPos endPos = route.get(route.size() - 1).toBlockPos();
        double distance = Math.sqrt(mc.player.distanceToSqr(endPos.getX() + 0.5, endPos.getY() + 0.5, endPos.getZ() + 0.5));
        String endLabel = "Â§l" + name + "Â§r Â§7(" + String.format("%.1f", distance) + "m)";
        RenderUtil.drawText(endLabel, endPos.getX() + 0.5, endPos.getY() + 2.2, endPos.getZ() + 0.5, 0.8F);
    }
    ...
    protected void onHudRender(GuiGraphics GuiGraphics) {
        ...
        infoLines.add("Â§6Â§l[Commission Debug Mode]Â§r");
        infoLines.add("");

        if (calculatedRoutes.isEmpty()) {
            infoLines.add("Â§cNo routes calculated");
            infoLines.add("Â§7Make sure you're in Dwarven Mines");
        } else {
            infoLines.add("Â§aRoutes Calculated: Â§f" + calculatedRoutes.size());
            infoLines.add("");
            ...
            for (Map.Entry<Commission, Route> entry : calculatedRoutes.entrySet()) {
                ...
                infoLines.add(colorCode + "â— Â§f" + commission.name() + " Â§7- " +
        ...
        infoLines.add("");
        if (simulatedTargetBlock != null) {
            infoLines.add("Â§6Target Block Found:");
            infoLines.add("Â§f " + simulatedTargetBlock.getX() + ", " + simulatedTargetBlock.getY() + ", " + simulatedTargetBlock.getZ());
            infoLines.add("Â§7 " + simulatedTargetReason);
            infoLines.add("Â§bCandidates: Â§f" + debugCandidates.size() + " Â§cObstructed: Â§f" + debugObstructed.size());
        } else {
            infoLines.add("Â§7No mineable blocks in range");
            if (!simulatedTargetReason.isEmpty()) infoLines.add("Â§8(" + simulatedTargetReason + ")");
        }

        infoLines.add("");
        infoLines.add("Â§7Press Â§e" + getKeybindName() + "Â§7 to toggle");

        RenderUtil.drawMultiLineText(GuiGraphics, infoLines, Color.WHITE, 0.8F);
    }

    private String getColorCode(Color color) {
        if (color.getRed() > 200 && color.getGreen() < 150 && color.getBlue() < 150) return "Â§c";
        if (color.getGreen() > 200 && color.getRed() < 150 && color.getBlue() < 150) return "Â§a";
        if (color.getBlue() > 200 && color.getRed() < 150 && color.getGreen() < 150) return "Â§9";
        if (color.getRed() > 200 && color.getGreen() > 200 && color.getBlue() < 150) return "Â§e";
        if (color.getRed() > 200 && color.getBlue() > 200 && color.getGreen() < 150) return "Â§d";
        if (color.getGreen() > 200 && color.getBlue() > 200 && color.getRed() < 150) return "Â§b";
        if (color.getRed() > 200 && color.getGreen() > 100 && color.getBlue() < 50) return "Â§6";
        return "Â§5";
    }

    private String getKeybindName() {
        try {
            int keyCode = Vertex.config().debug.commissionDebugKeybind;
