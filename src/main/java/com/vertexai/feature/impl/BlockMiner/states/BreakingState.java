...
public class BreakingState implements BlockMinerState {
    ...
    public BlockMinerState onTick(BlockMiner miner) {
        ...
        // After breaking a block, check if we need to stop or transition
        ...
        if (!detectedBlockType.equals(miner.getTargetBlockType())) {
            ...
            if (com.vertexai.Vertex.config().miningMacro.oreType == 0) { // Mithril Macro
                shouldReset = false; // By default, keep swinging through lag blocks
                
                if (detectedBlockType == net.minecraft.world.level.block.Blocks.BEDROCK || detectedBlockType == net.minecraft.world.level.block.Blocks.AIR) {
                    shouldReset = true;
                } else if (detectedBlockType == net.minecraft.world.level.block.Blocks.POLISHED_DIORITE) {
                    if (com.vertexai.Vertex.config().miningMacro.mineTitanium) {
                        // Seamlessly transition to mining the Titanium
                        miner.setTargetBlockType(detectedBlockType);
                        this.miningTime = BlockUtil.getMiningTime(mc.level.getBlockState(miner.getTargetBlockPos()), miner.getMiningSpeed());
                        this.breakAttemptTime = 0;
                        shouldReset = false;
                    } else {
                        // We don't want to mine Titanium, so stop here
                        shouldReset = true;
                    }
                } else if (detectedBlockType == net.minecraft.world.level.block.Blocks.STONE || detectedBlockType == net.minecraft.world.level.block.Blocks.COBBLESTONE) {
                    shouldReset = true;
                }
            }
    ...
    private void handleWalking() {
        ...
        if (walkingDistance > MIN_WALK_DISTANCE && miningDistance > MAX_MINE_DISTANCE) {
            KeyBindUtil.holdThese(mc.options.keyUp);
        } else {
