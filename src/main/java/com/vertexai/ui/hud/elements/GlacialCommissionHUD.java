...
public class GlacialCommissionHUD extends TextHud {
   ...
   protected boolean shouldShow() {
      ...
      if (!glacialMacro.isEnabled()) return false;
      boolean macroTypeCondition = (Vertex.config().general.macroType == 1);
      SubLocation currentSub = GameStateHandler.getInstance().getCurrentSubLocation();
      boolean locationCondition = currentSub == SubLocation.GLACITE_TUNNELS || currentSub == SubLocation.DWARVEN_BASE_CAMP || Vertex.config().commission.glacialCommission.showGlacialHUDOutside;
