...
public class RouteHandler {
    ...
    public synchronized void savePathfinderData() {
        while (com.vertexai.macro.features.navigation.PathfinderRouteBuilder.getInstance().isRunning()) {
            try {
                boolean shouldSave;
                synchronized (saveLock) {
                    while (com.vertexai.macro.features.navigation.PathfinderRouteBuilder.getInstance().isRunning() && !this.pathfinderDirty) {
                        saveLock.wait(500L);
                    }
                    if (!com.vertexai.macro.features.navigation.PathfinderRouteBuilder.getInstance().isRunning()) {
                        break;
                    }
    ...
    public void onRender(WorldRenderContextWrapper context) {
        ...
        if (com.vertexai.macro.features.navigation.PathfinderRouteBuilder.getInstance().isRunning() && this.selectedPathfinderRoute != null && !this.selectedPathfinderRoute.isEmpty()) {
