...
public class AudioManager {
    ...
    public void resetSound() {
        ...
        if (Vertex.config().failsafe.maxOutMinecraftSounds) {
            mc.options.getSoundSourceOptionInstance(SoundSource.MASTER).set((double) soundBeforeChange);
        }
    }

    public void playSound() {
        if (Vertex.config().failsafe.failsafeSoundType == 0) {
    ...
    private void playCustomSound() {
        Vertex.executor().execute(() -> {
            try {
                ...
                setSoundVolume(Vertex.config().failsafe.failsafeSoundVolume);
                clip.start();
    ...
    private AudioInputStream getAudioStreamForSelectedSound() throws Exception {
        switch (Vertex.config().failsafe.failsafeSoundSelected) {
            case 0: // Custom sound file
                File audioFile = new File(mc.gameDirectory.getAbsolutePath() + "/Vertex_sound.wav");
                if (audioFile.exists() && audioFile.isFile()) {
                    return AudioSystem.getAudioInputStream(audioFile);
                }
                break;
            case 1:
                return AudioSystem.getAudioInputStream(getClass().getResource("/Vertex/sounds/staff_check_voice_notification.wav"));
            case 2:
                return AudioSystem.getAudioInputStream(getClass().getResource("/Vertex/sounds/metal_pipe.wav"));
            case 3:
                return AudioSystem.getAudioInputStream(getClass().getResource("/Vertex/sounds/AAAAAAAAAA.wav"));
            case 4:
                return AudioSystem.getAudioInputStream(getClass().getResource("/Vertex/sounds/loud_buzz.wav"));
            default:
    ...
    public void onTick() {
        ...
        if (Vertex.config().failsafe.failsafeSoundType != 0 || !minecraftSoundEnabled) return;
    ...
    private void handleMinecraftSoundTick() {
        ...
        net.minecraft.sounds.SoundEvent event = Vertex.config().failsafe.failsafeMcSoundSelected == 0 ? SoundEvents.EXPERIENCE_ORB_PICKUP : SoundEvents.ANVIL_LAND;
