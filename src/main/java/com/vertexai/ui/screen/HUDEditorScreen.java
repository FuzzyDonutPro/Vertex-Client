package com.vertexai.ui.screen;
...
public class HUDEditorScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        for (AbstractHUDElement element : elements) {
            ...
            int outlineColor = isDragging ? 0x40FFB000 : (isSelected ? 0x4022D3EE : 0x20FFFFFF);
            context.fill(x0, y0, x0 + w, y0 + h, outlineColor);
            if (isSelected && !isDragging) {
                context.fill(x0 - 1, y0 - 1, x0 + w + 1, y0 + h + 1, 0x8022D3EE);
            }
            ...
            if (hovered || isSelected || isDragging) {
                ...
                context.fill(x0 - handle, y0 - handle, x0 + handle + 1, y0 + handle + 1, handleBorder);

                context.fill(x0 + w - handle, y0 - handle, x0 + w + handle + 1, y0 + handle + 1, handleFill);
                context.fill(x0 + w - handle, y0 - handle, x0 + w + handle + 1, y0 + handle + 1, handleBorder);

                context.fill(x0 - handle, y0 + h - handle, x0 + handle + 1, y0 + h + handle + 1, handleFill);
                context.fill(x0 - handle, y0 + h - handle, x0 + handle + 1, y0 + h + handle + 1, handleBorder);

                context.fill(x0 + w - handle, y0 + h - handle, x0 + w + handle + 1, y0 + h + handle + 1, handleFill);
                context.fill(x0 + w - handle, y0 + h - handle, x0 + w + handle + 1, y0 + h + handle + 1, handleBorder);
