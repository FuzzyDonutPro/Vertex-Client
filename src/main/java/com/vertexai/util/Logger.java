...
public abstract class Logger {
    ...
    public static void sendMessage(final String message) {
        addMessage(formatPrefix("§bVertex", message));
        Vertex.LOGGER.info("{}", message == null ? "null" : ChatFormatting.stripFormatting(message));
    }

    public static void sendWarning(final String message) {
        addMessage("§c§l[WARNING] §8» §e" + message);
        Vertex.LOGGER.warn("{}", message == null ? "null" : ChatFormatting.stripFormatting(message));
    }

    public static void sendError(final String message) {
        addMessage("§l§4§kZ§r§l§4[Vertex]§kH§r §8» §c" + message);
        Vertex.LOGGER.error("{}", message == null ? "null" : ChatFormatting.stripFormatting(message));
    }
    ...
    public static void sendLog(final String message) {
        ...
        if (Vertex.config() != null && Vertex.config().debug.debugMode && mc.player != null) {
            addMessage("§l§2[Vertex] §8» §7" + message);
            Vertex.LOGGER.debug("{}", message == null ? "null" : ChatFormatting.stripFormatting(message));
        } else {
            Vertex.LOGGER.info("{}", message == null ? "null" : ChatFormatting.stripFormatting(message));
        }
    }
